...
class EORepoActor  (eomodelActor: ActorRef, ws: WSClient) extends Actor with ActorLogging {
  ...
  def newEO(entityName: String, eos: List[EO]): Future[EO] = {
    Logger.debug("New EO: " + eos)
    val eo = eos.head
    ...
    Logger.debug("New EO | Upate WS:  " + url)
    Logger.debug("New EO | WS post data: " + data)
    ...
    val result = futureResponse.map( response => {
      ...
      /*   } catch {
        case parseException: JsonParseException => {
          println("parseException " + parseException)
          handleException(response.body, eo)
        }
        case t: Throwable => {
          println("Throwable " + t)
  ...
  def saveEO(eo: EO): Future[EO] = {
    ...
    futureResponse.map { response =>
      try {
        Logger.debug("Save EO | get response: " + response)
        val resultBody = response.json
  ...
  def receive = LoggingReceive {
    ...
    case NewEO(d2wContext: D2WContext, eos: List[EO], ruleResults: Option[List[RuleResult]], requester: ActorRef) =>
      ...
      val newEOV = newEO(entityName, eos)
      log.debug("NewEO | newEOV " + newEOV)

      newEOV.map(rrs => {
        log.debug("NewEO | will send SavingResponse " + rrs)
    ...
    case UpdateEO(d2wContext: D2WContext, eos: List[EO], ruleResults: Option[List[RuleResult]], requester: ActorRef) =>
      ...
      updateEO(eos).map(rrs => {
        println("EORepoActor will send SavingResponse")
